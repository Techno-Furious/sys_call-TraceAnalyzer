...
const char *syscall_names[] = {
    ...
    "setregid", "getgroups", "setgroups", "setresuid", "getresuid", "setresgid", "getresgid",
    ...
    "name_to_handle_at", "open_by_handle_at", "clock_adjtime", "syncfs", "sendmmsg",
    ...
    "statx", "io_pgetevents", "rseq", "pidfd_send_signal", "io_uring_setup", "io_uring_enter",
    "io_uring_register", "open_tree", "move_mount", "fsopen", "fsconfig", "fsmount",
    "fspick", "pidfd_open", "clone3", "close_range", "openat2", "pidfd_getfd", "faccessat2",
    "process_madvise", "epoll_pwait2", "mount_setattr", "quotactl_fd", "landlock_create_ruleset",
    "landlock_add_rule", "landlock_restrict_self"
};

#define MAX_SYSCALLS (sizeof(syscall_names) / sizeof(syscall_names[0]))

void trace_syscall(pid_t child, FILE *log_file) {
    int status;
    ...
    while (1) {
        ...
        if (syscall_num >= 0 && syscall_num < MAX_SYSCALLS) {
            ...
            fprintf(log_file, "0x%llx, 0x%llx, 0x%llx, 0x%llx, 0x%llx, 0x%llx",
                    regs.rdi, regs.rsi, regs.rdx, regs.r10, regs.r8, regs.r9);

            fprintf(log_file, ") = ");
        } else {
            fprintf(log_file, "unknown_syscall(%ld) = ", syscall_num);
        }
...
int main(int argc, char *argv[]) {
    ...
    FILE *log_file = fopen("syscallLogs.txt", "w");
    if (!log_file) {
        perror("fopen");
        exit(EXIT_FAILURE);
    }
